                                ...
                                @foreach ($Reclamations as $reclamation )
                                   <tr>
                                        ...
                                        <th class="w-auto d-flex justify-content-end ">
                                            ...
                                            @if($User->IsAdmin == 0)
                                                <a href="{{ route('reclamation.ReclamationPdf' ,$reclamation->id ) }}"
                                                    <button type="button" class="btn btn-primary  text-black"  id="btn-update" >PDF </button>
                                                </a>
                                            @endif
                                        ...
                                        @else
                                         <button data-bs-toggle="modal" data-bs-target="#staticBackdrop{{ $reclamation->id  }}" id="basic-addon2" type="submit" class="btn btn-success  text-black">Suprimer</button>
                                                                                <!-- Modal -->
                                        <div class="modal fade" id="staticBackdrop{{ $reclamation->id  }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="staticBackdropLabel">Clarification Reclamation #{{ $reclamation->id  }} </h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                        <!-- Form Ajoute Reclamation -->
                                                <form class="needs-validation" method="POST" action="{{ route('reclamation.ReclamationDel') }}" >
                                                    @csrf
                                                    <div class="mb-3">
                                                        <input type="hidden" name="reclamation_id" value="{{ $reclamation->id }} " id="">
                                                        <label for="exampleFormControlInput1" class="form-label">Cause</label>
                                                        <input type="text" class="form-control" id="exampleFormControlInput1" name="Cause" placeholder="name">
                                                        @error('Name')
